' Transpiled on 29-04-2025 20:52:15
...
Dim m(80*512\4) ' physical z-machine memory
Dim pf                                ' counter for number of page faults
Dim MEM_FILE$                         ' the z-machine file
Dim pp_to_vp(80 - 1)  ' map of physical pages -> virtual pages
Dim vp_to_pp(256 - 1)   ' map of virtual pages -> physical pages
...
Function mem_load(vp)
  ...
  Inc next_page
  If next_page = 80 Then next_page = FIRST_SWAP_PAGE
...
Sub mem_init(f$)
  ...
  FIRST_SWAP_PAGE = BASE_STATIC \ 512
  If BASE_STATIC Mod 512 > 0 Then FIRST_SWAP_PAGE = FIRST_SWAP_PAGE + 1
...
Function vget(i)
  If i = 0 Then
    vget = Peek(Short stack.base + sp * 2 - 2) : Inc sp, -1
  ElseIf i < &h10 Then
    vget = Peek(Short stack.base + (fp + i + 4) * 2)
  ElseIf i <= &hFF Then
...
Sub vset(i, v)
  If i = 0 Then
    Poke Short stack.base + sp * 2, v : Inc sp
  ElseIf i < &h10 Then
    Poke Short stack.base + (fp + i + 4) * 2, v
  ElseIf i <= &hFF Then
...
Function ob_prop_base(o)
  ...
  ob_prop_base = rb(ad) * 256 + rb(ad + 1)
End Function
...
Function ob_prop_get(o, p)
  ...
  If ad > 0 Then
    ...
    If sz = 2 Then ob_prop_get = rb(ad) * 256 + rb(ad + 1) : Exit Function
    Error "Property length > 2"
  EndIf
  ad = rw(&h0A) + 2 * (p - 1)
  ob_prop_get = rb(ad) * 256 + rb(ad + 1)
End Function
...
Sub ob_prop_set(o, p, x)
  ...
  Select Case ob_prop_len(ad)
    Case 1
      wb(ad, x And &hFF)
    Case 2
      Poke Var m(0), ad, x \ 256 : Poke Var m(0), ad + 1, x Mod 256
    Case Else
...
Function ex_2op()
  ...
  ElseIf oc = &hD Then
    ...
    If a = 0 Then Poke Short stack.base + sp * 2 - 2, b Else vset(a, b)
...
Function ex_1op()
  ...
  ElseIf oc = &hE Then
    ' LOAD
    ' Note special handing for op with indirect reference to the stack pointer
    If a = 0 Then x = Peek(Short stack.base + sp * 2 - 2) Else x = vget(a)
    vset(st, x)
...
Function ex_0op()
  ...
  ElseIf oc = &h8 Then
    ' RET_POPPED
    x = Peek(Short stack.base + sp * 2 - 2) : Inc sp, -1
    ex_return(x)
...
Function ex_varop()
  ...
  ElseIf oc = &h8 Then
    ' PUSH
    Poke Short stack.base + sp * 2, oa(0) : Inc sp

  ElseIf oc = &h9 Then
    ' PULL
    x = Peek(Short stack.base + sp * 2 - 2) : Inc sp, -1
    ' Note special handing for op with indirect reference to the stack pointer
    If oa(0) = 0 Then Poke Short stack.base + sp * 2 - 2, x Else vset(oa(0), x)
...
Sub ex_return(x)
  ...
  pc = &h10000 * Peek(Short stack.base + sp * 2 - 2) : Inc sp, -1
  Inc pc, Peek(Short stack.base + sp * 2 - 2) : Inc sp, -1
  st = Peek(Short stack.base + sp * 2 - 2) : Inc sp, -1
  fp = Peek(Short stack.base + sp * 2 - 2) : Inc sp, -1
  vset(st, x)
...
Sub ex_call(st)
  ...
  Poke Short stack.base + sp * 2, fp : Inc sp
  fp = sp - 1
  Poke Short stack.base + sp * 2, st : Inc sp
  st_push(pc Mod &h10000)
  st_push(pc \ &h10000)
  pc = 2 * oa(0)
  nl = rp() ' number of local variables
  Poke Short stack.base + sp * 2, nl : Inc sp
  For i = 1 To nl
    x = rp() * 256 + rp()
...
Sub dmp_mmap
  ...
  nc = 6 : nr = CInt(80 / nc + 0.4999) : width = 15
  For i = 0 To nr - 1
    For j = 0 To nc - 1
      con.flush()
      x = (j * nr) + i
